...
void CTool_Dungeon::Get_MousePos_On_Dungeon()
{
	...
	if (isSuccess)
	{
		...
		_int iIndex = m_pNavigationCom->Find_Index(ptMouse, vMousePos, m_pGameInstance->Get_Z(), m_pTransformCom->Get_WorldMatrix());
...
_bool CTool_Dungeon::isRemove()
{
	...
	if (isSuccess)
	{
		...
		GetCursorPos(&ptMouse);
		ScreenToClient(g_hWnd, &ptMouse);
		iIndex = m_pNavigationCom->Find_Index(ptMouse, vMousePos, m_pGameInstance->Get_Z(), m_pTransformCom->Get_WorldMatrix(), &iVecterIndex);
	}
	else
		return false;
	//셀 지우기
