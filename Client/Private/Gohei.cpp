...
CGohei::CGohei(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CSkillObj{ pDevice, pContext }
{
}

CGohei::CGohei(const CGohei& rhs)
	: CSkillObj{ rhs }
{
}

HRESULT CGohei::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CGohei::Initialize(void* pArg)
{
...
void CGohei::Priority_Tick(const _float& fTimeDelta)
{
}

void CGohei::Tick(const _float& fTimeDelta)
{
...
void CGohei::Late_Tick(const _float& fTimeDelta)
{
	_matrix		SocketMatrix = XMMatrixIdentity();

	m_isHide = !(*m_pState == PLAYER_ATTACK_1
...
HRESULT CGohei::Bind_ResourceData()
{
...
void CGohei::Change_Animation(const _float& fTimeDelta)
{
...
CGohei* CGohei::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CGohei* pInstance = new CGohei(pDevice, pContext);
...
CGameObject* CGohei::Clone(void* pArg)
{
	CGohei* pInstance = new CGohei(*this);
