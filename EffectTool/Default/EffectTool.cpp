                     ...
                     _In_ int       nCmdShow)
{
    ...
    CMainEffect* pMainEffect = CMainEffect::Create();
    if (nullptr == pMainEffect)
        return FALSE;

    if (FAILED(pGameInstance->Ready_Timer(L"Timer_Default")))
        return FALSE;
    if (FAILED(pGameInstance->Ready_Timer(L"Timer_60")))
        return FALSE;
    ...
    while (TRUE)
    {
        ...
        if (fTimeAcc > 1.f / 60.0f)
        {
            ...
            pMainEffect->Tick(pGameInstance->Get_TimeDelta(TEXT("Timer_60")));
            pMainEffect->Render();
